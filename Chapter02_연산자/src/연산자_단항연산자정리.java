 ...
 * 		= 한개	(변수)		  수정 / 삭제 / 읽기
 * 		-------------------     =>방식은 제어문, 연산자 가 있다
 * 		= 여러개	(배열, 클래스)		=> 변수 / 제어문 / 연산자 가 핵심이다.
 * 		-------------------		같은 기능별 묶음
 * 		=RAM에 저장 (휘발성 메모리)	    게시판,댓글,회원가입,예약....
 * 			=> 프로그램 종료시 자동으로 사라진다         ↘ 재사용을 위해 기능별로 묶어놓는다
 * 		-------------------
 * 		=영구적인 저장 장치
 * 			=> 파일
 * 			=> RDBMS (오라클) (관계형 데이터베이스)  -오라클의 장점:사라지지 않는다
 * 		-------------------	
 * 		연산처리
 * 
 * 		1) 단항연산자
 ...
 * 				** 각 문자마다 번호 => 아스키 코드값 이라고 부름
 * 			    (데이터형) 값
 * 				--------- 변경
 * 			= 반전연산자 => 양수 => 음수 변경 (~)
 * 		2) 이항연산자
 * 			=연산 대상이 2개인 경우
 * 			 산술 연산자 => 같은 데이터형을 만들어 놓고 시작한다 (같은 데이터형 끼리만 연산 가능)
 * 			---------
 *			+ , - , * , / , %
 *			 => + 
 *            일반 산술 (더하기)
 ...
 *			 => - , * 빼기나 곱셈만 가능
 *			 => /
 *			  1) 0으로 나눌 수 없다 (사용자가 0을 임력할 수 없게 제어)
...
public class 연산자_단항연산자정리 {
	...
	public static void main(String[] args) {
		...
		System.out.println("b4=" + b4); //초과하면 답이 달라지기 때문에 byte+byte=int로 받아라
