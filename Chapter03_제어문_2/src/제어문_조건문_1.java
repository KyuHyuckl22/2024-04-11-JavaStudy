 ...
 *				{
 *					조건이 true일 경우에만 수행되는 문장.
 *				}
 *			 => 부정연산자(!) => 반대 효과
 *		 	 => 논리연산자 => 조건이 2개인 경우
 *				->id가 같고 비밀번호가 같은경우 로그인 -> && , ||
 *			 => 비교연산자 (== , != , < , > , <= , >=)
 *			 => 프로그램 : 라이브러리 사용 / 데이터베이스(오라클) => 데이터 처리
 *				-----------------------------------------------
 *				사용자가 요청한 데이터만 읽기 => 출력
 *				-----------------> 요청한 것만 읽으려다 보니
 *				|조건이 걸림 => 사용자 요청 에 의해 처리 결과물을 보여줌 => 사용자중심의 프로그램
 ...
 *			3. 다중 조건문 =>여러개의 조건중에 한개만 처리할때
 *			4. 선택문 => 값 1개를 선택해서 처리 => 메뉴, 키보드(게임)등 에서 주로 사용
 ...
 *
 *	단일 조건문
 *		: 자바에서 제어문은 바로 밑에있는 문장만 제어할 수 있다.
 *		=>if(조건문)
 *			실행문장1   ==> if문 소속문장
 ...
 * 
 */
// 숫자 입력 => 같은 숫자인지 확인 => 11 22 33 44 ... 10의자리와 1의자리가 같은수 인지
...
//			}	
//		}	
		if(!(num>=10 && num<=99)) //오류처리 => 사용자가 잘못 입력한 경우
		{
