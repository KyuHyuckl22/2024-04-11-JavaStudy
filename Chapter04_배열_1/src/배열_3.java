 ...
 *        데이터형 배열명[]; //C언어        
 *        
 *        => 데이터형 : 배열에 저장되는 데이터 종류 (어떤 종류를 저장할것인가)
 *           정수 : int[]
 *           실수 : double[]
 *           문자열: String[]
 *           ...
 *           문자 : char[]
 *           => [] => 같은 데이터를 모아서 관리 => 배열기호 
 *           => 배열명은 실제 값을 가지고있는 것이 아니고, 실제 데이터가 저장되어있는 위치에 주소값
 *              ----- 주소 참조해서 여러개의 데이터 관리 => 참조 변수
 *     => 기본값으로 초기화
 *        데이터형[] 배열명 = new 데이터형[갯수]
 *        ------------------------------
 *        int = 0 , double = 0.0  boolean = false
 *        int[] arr=new  double[3] ==>0.0 0.0 0.0 
