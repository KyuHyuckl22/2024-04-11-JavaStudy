...
public class 메소드_8 {
	static void find(String fd){
		 ...
		 for(String t: title) {
			 if(t.contains(fd)) { //검색어가 저 타이틀 안에 있는가
...
/*
 *          int add(int a, int b)  => 값을 보낼때 가급적이면 같은 데이터형을 사용하라  
 *          
 *           
 *          => int a = add() => byte/char
 *            ----
 *            long / float / double   큰것
 *            작은거로 바꾸고싶다면 형 변환이 필요하다
 */
	public static void main(String[] args) {
		...
		String fd=scan.next();
		find(fd);  //void 일때 (받은값을 모를때) 호출하는 메소드
		/*
		 *   리턴형
		 *   데이터형 변수 = 메소드명 (데이터형에 맞는 실제값)
		 */
//		double java.lang.Math.random()
//		리턴형                      매개변수    앞리뒤매 꼭 기억하기
//		매개변수가 있고 없고 에도 메서드가 구분된다 
//	ex) 
//		find(String fd)  와
//		find() 는 다른 메서드가 된다
//		int r=Math.random();   //  Math.random 은 0.0~ 0.99 사이의 double 형이라 int 로 형 변환 후 사용 했던것
