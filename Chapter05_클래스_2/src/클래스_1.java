 ...
 *   프로그램
 *   
 *   요구사항 분석 ====> 데이터베이스 설계 ====> 화면UI ====> 구현시작 ==> 테스트 ==> 배포
 *    1) 필요한 데이터   데이터값을 저장                       |                 |     
 *    2) 기능 설정                                     웹 프로그래머          발표   
 *   --------------- 클래스 (설계)
 *    => 프로그램에 필요한 데이터 => 변수 라고 부른다
 *    => 기능설정            => 메소드 라고 부른다
 *   ---------------    
 *    결국 프로그램은
 *    변수의 집합  +  메소드 
 *      |  
 *    초기값 => 생성자
 *   ---------------  관리하는 영역 ==> (객체) ==> 메모리에 저장후 사용
 *                                            -------------- 객체
 *    현실세계에 있는 모든 사물 => 단순화
 *     Ex) 
 *        사원을 만든다 => 시물레이션 (사원에는 이런게 있다라는 (가정))
 *         => 프로그래머가 알 수 있게 설계
 *          사번 / 이름 / 부서명 / 직위 / 입사일 / 연봉 / 근무지 등.. => 변수 => 한명에 대한 설계
 *          출근한다 / 퇴근한다 / 점심을 먹는다 ... (변수를 갖고 활용을 하는것)메소드
 *     1) 객체 지향 프로그램 을 잘 하기 위해선
 *        1. 변수 설정을 잘 해야 한다 => 어떤 데이터를 관리 할것인지           
 *        2. 기능 => 메뉴
 *     2) 설계
 ...
 *        변수묶음
 *         1. 변수의 종류
 *           = 객체변수 => new를 이용해서 저장시마다 메모리가 달라짐(저장 공간이 따로 생긴다)
 ...
 *               
 *               ----arr---
 *                  0x100       0x100 의 첫번째 값 | 0x100 의 두번째 값
 *                                     arr[0]       arr[1]  => 인덱스를 이용해 첫번째 값과 두번째 값을 표현 -> 메모리크기가 동일해야함 
 *               ----------           --------------------
 *                                        1     |    2
 *                                   |--------------------
 *                                 0x100
 *                                 
 *               ** 데이터를 설계 => 메모리크기가 다른것이 있다
 *               
 *               1. 클래스 저장 
 *                  A a=new A()
 *                      ---
 ...
 *                                             0 ==> 초기값을 저장해주는 역할
 *                                          --------
 *                  A b=new A()  => new를 이용 => 메모리가 따로 생성이 되기 때문에 접근하는 범위는 해당 주소에서만 사용이 가능
 *                      ---                                                   다른 주소에서는 사용이 불가능
 *                   --- b ---
 *                   ---------  => 0x200    --------
 *                                            ---aa
 ...
 *                                             0
 *                                          --------
 *                  => new는 사용할때마다 따로 메모리 생성 (클래스 크기만큼) => 제한(하드디스크 크기만큼 사용이 가능)
 *                  => new를 이용해서 저장되는 내용
 ...
 *                   (계산기 여러개를 켰을때 하나에 9를 줬는데 나머지도 9가 나온다면 static 아니라면 인스턴스)
 ...
 *                         => 로그인 버튼, 계산기 버튼 ... => 버튼을 누를때마다 호출은 가능
 *                   => 인터페이스
 *                      ----------다른 클래스 연결시에 주로 사용
 *                      스프링 <=> 오라클
 *                      스프링 <=> 자바스크립트
