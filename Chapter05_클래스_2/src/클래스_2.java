/*
 *     1. => 114p 
 *           객체 => 변수 (여러개의 변수, 여러개의 메소드) 동시에 관리하는 영역
 *           --- 설계 => 영억 class
 *           --- 동시에 관리를 위해서는 저장 => new 
 *     2. => 115p
 *     Ex) 학생 설계 같은 학교일 경우
 *       학번 / 이름 / 학교명 / 성별 / 주소 ... 변수
 *       --------   -----  ---------
 *                    |  (학교명)한개의 이름만 가지고 있는것 
 *          |                |
 *          -----------------
 *                  | 학생마다 따로 가지고 있는 변수 => new 따로 저장하는 공간을 만들어 준다
 *                  
 *          => 명사적  -> 변수
 *          
 *          => 동사적  -> 메소드 : 점심을 먹는다, 시험을 본다, 게임을 한다...( 동작하는것들 )
 *          
 *          변수 / 변수를 활용하는 메소드 ==> 클래스 -> 관리하는 영역을 객채라고 한다.
 *          => 예) 
 *               시계
 *                시침 / 분침 / 초침
 *             Ex)
 *               계산기
 *               첫번째 정수 / 두번째 정수 / 연산자
 *          => ** 객체란 현실세계 / 비현실 세계를 특성화 시킨 것이다
 *                                        ----- 추상화 ---> 단순화 
 *           (기술면접 : 객체란 무엇인가? 현실세계 / 비현실 세계 를 변수와 메서드를 사용해 단순화 시킨것이다)
 *           
 *           벤치마킹 (사이트 결정) ===> 분석 (명사적, 동사적)  ==> 클래스 설계
 *                                       |     |             필요한 데이터 (변수), 사용자 동작(메소드)
 *                                      출력물   클릭/메뉴 선택        |
 *                                                            공통으로 사용되는 변수 : static
 *                                                            따로 사용되는 변수 : 인스턴스
 *      115p
 *            클래스 명 = 변수 식별자와 동일
 *            ---------------------
 *             알파벳이나 한글로 시작한다 (알파벳은 대소문자를 구분함)
 *             특수문자 사용가능 _ , $ (주로 _를 사용 => 단어 2개인 경우, 임시 클래스
 *                                  Main_Class   ,   File_Name , _Temp
 *             숫자 사용이 가능(앞에사용 금지)
 *             공백은 사용이 불가능
 *             키워드는 사용할 수 없다
 *             =====
 *             약속 
 *              => 한글은 가급적 사용하지 않는다 -> 한글은 운영체제에 따라 다르다 -> 알파벳을 이용한다
 *                 한글로 제작 => AWS에 호스팅 => 한글이깨진다
 *               
 *              => 알파벳은 시작 첫 문자는 대문자로 한다
 *                                   ---------- 대문자로 하지 않으면 초짜로 보임
 *              => 클래스명, 변수가 없는데 호출, 들여쓰기
 *                
 *              => 같은 폴더에서는 같은 이름의 클래스명을 사용할 수 없다)
 *              => 한개의 파일(.java) 에 여러개의 클래스를 만들 수 있다
 *              => 클래스의 형태
 *                = 데이터형 클래스
 *                   => 변수만 설정 => 사용자 정의 데이터형
 *                                 ~VO , ~DTO(myBatis), ~Bean(JSP)
 *                                 
 *                = 액션 클래스
 *                   => 메소드 설정 => 기능만 처리
 *                                  ~DAO, ~Manager, ~Controller (요청 => 응답)
 *           117p ~ 118p  8장까지 책 정독하기
 *           class ClassName
 *           {
 *              **** 멤버변수 
 *                   new => 저장 => 따로 메모리 생성  => 인스턴스 변수
 *                   컴파일시 저장 => 한개의 메모리 생성  => 정적변수 ==> stati
 *              **** 초기화
 *                   인스턴스 변수 : {} , 생성자
 *                   static 변수 : static{} , 생성자 => 자동 저장이 가능 => 생성자를 이용하지 않는다
 *              **** 변수활용 -> 메소드     
 *           }
 *            
 *             
 */
public class 클래스_2 {

	public static void main(String[] args) {
		
	}
}
