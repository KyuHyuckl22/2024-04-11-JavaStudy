...
class RecipeSystem {
	Recipe2[] recipes=new Recipe2[40]; //선언만 가능한 곳 => 외부에서 읽기 불가능
//	 초기화 블록 이 아니면 생성자를 사용해야 함                     ---------= 외부에서 읽는 것을 구현 이라고함
//	 외부에서 데이터를 얻는 경우 / 파일 데이터 읽기
	{
//		배열에 데이터 초기화
...
//			Element  => 태그 <div> => 여러개 s를 붙여줌 => Elements
