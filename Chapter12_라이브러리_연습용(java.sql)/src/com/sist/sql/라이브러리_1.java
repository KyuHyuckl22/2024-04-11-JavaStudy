...
// => 오라클 연결 해주는게 ojdbc8.jar => 데이터베이스 연결시 해당 업체에서 다운로드
...
public class 라이브러리_1 {
	...
	public static void main(String[] args) {
		try {
			...
			Class.forName("oracle.jdbc.driver.OracleDriver"); // 메모리 할당 => 리플렉션 => new를 줄여나가야 함
			// 2. 오라클 연결
...
//				 출력할           첫번째 데이터 정수형  두번째 데이터 문자열  세번째 데이터 문자열  네번째 데이터 정수형 
			}
				
			// 5. 브라우저 전송 => (우린 브라우져가 없으니 도스창에 출력)
		}catch(Exception ex) {
			ex.printStackTrace();
		}
	}
