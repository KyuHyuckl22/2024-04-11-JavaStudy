 ...
 *     -------------------
 *            클래스화         데이터형을 제어하는 클래스 : Wrapper class
 *     int      -> Integer   
 *     long     -> Long
 *     byte     -> Byte
 *     double   -> Double
 ...
 *          => replaceAll()
 *          => split()
 *          => concat() => 문자열 결합시 사용 (+ 가 있기때문에 사용 빈도가 거의 없음)
 *        제어
 *          => substring()
 *          => indexOf()
 *          => lastIndexOf()
...
//			노래 출력
			 System.out.println("등록된 곡 총 "+title.length+"곡");
			int rank=1;
			for(String t:title)
			{
			...
			System.out.println("================================================");
//			노래검색  => contains --> 내가 적은 단어들을 찾는 (검색) 역할 ->Oracle => LIKE
			...
			for(String t:title)
			{
				if(t.contains(fd)) // 제목안에 단어가 포함되어 있는지  --> 결과값은 true / false
				{
